...
export const createPortfolioApiFirebase = (updatedPortfolioData: any) => async (dispatch: any) => {
...
const updatePortfolioApiFirebase = (updatedPortfolioData: any) => async (dispatch: any) => {
    const userId = auth?.currentUser?.uid
    const portfolioDocRef = doc(portfolioCollectionRef, userId)
    try {
...
export const deleteCoinFromPortfolioApiFirebase = (coinId: any) => async (dispatch: any) => {
    const userId = auth?.currentUser?.uid
    const portfolioDocRef = doc(portfolioCollectionRef, userId)
    const documentSnapShot = await getDoc(portfolioDocRef)
    if (documentSnapShot) {
        ...
        const filteredData = currentPortfolio.filter((item: any) => item.id !== coinId)
...
export const updatePortfolioThunk = (id: any, currentCoinPrice: any, totalBuyingAmount: any, coinQuantity: any) => async (dispatch: any, getState: any) => {
...
export const createNewCoinInPortfolioThunk = (id: any, icon: any, rank: any, name: any, symbol: any, currentCoinPrice: any, totalBuyingAmount: any, coinQuantity: any) => async (dispatch: any, getState: any) => {
    const {myCurrentPortfolioDataFB} = getState().myPortfolio
    ...
    const updatedPortfolioData = myCurrentPortfolioDataFB.length === 1 && myCurrentPortfolioDataFB[0].id === ""
